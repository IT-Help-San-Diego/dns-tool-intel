...
PostgreSQL is the primary persistent store. The schema is defined by the versioned migration chain in `go-server/db/migrations/`, which is embedded in the binary and applied at startup — the server creates a database from empty and upgrades an existing one through the same path. `go-server/db/schema/schema.sql` is generated documentation (`pg_dump --schema-only` of that chain, regenerated by `./scripts/regen-schema-doc.sh`) and is never executed.

Queries are written in `go-server/db/queries/` and generated by `sqlc` into type-safe Go code in `go-server/internal/dbq/`.

Schema changes are made by adding a migration, never by editing an existing one: applied migrations are checksummed, and editing one after it has run is rejected at startup.
