...
(function(){
  // Fragment navigation + scrollIntoView silently no-op inside closed <details>.
  // Open every ancestor group before any in-page jump (persona-coverage finding).
  ...
  function goHash(){ var id=decodeURIComponent(location.hash.slice(1)); if(!id) return;
    var t=document.getElementById(id); if(t){ openFor(t); t.scrollIntoView(); } }
