...
type Entry struct {
	...
	TextFragments  map[string]string `yaml:"text_fragments,omitempty" json:"text_fragments,omitempty"`
}
...
func appendTextFragment(baseURL, section string, fragments map[string]string) string {
	if len(fragments) == 0 {
		return baseURL
	}
	frag, ok := fragments[section]
	if !ok || frag == "" {
		return baseURL
	}
...
func (r *Registry) ResolveRFC(id string) (rfcLabel string, rfcURL string) {
	...
	rfcURL = appendTextFragment(rfcURL, section, e.TextFragments)
...
func (r *Registry) ResolveSectionURL(id, section string) string {
	...
	if section == "" {
		return appendTextFragment(e.URL, "", e.TextFragments)
	}
	base := fmt.Sprintf("%s#section-%s", e.URL, section)
	return appendTextFragment(base, section, e.TextFragments)
}
