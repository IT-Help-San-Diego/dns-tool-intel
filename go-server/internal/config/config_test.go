...
func TestLoad_BaseURL_DeploymentKeepsCustomDomain(t *testing.T) {
        setEnv(t, "DATABASE_URL", "postgres://test")
        setEnv(t, "SESSION_SECRET", "test-secret")
        // A real custom domain set explicitly for the deployment is honored.
        setEnv(t, "BASE_URL", "https://staging.it-help.tech")
        setEnv(t, "GOOGLE_REDIRECT_URL", "")
        setEnv(t, "REPLIT_DEPLOYMENT", "1")
        os.Unsetenv("REPLIT_DEV_DOMAIN")
        t.Cleanup(func() { os.Unsetenv("REPLIT_DEPLOYMENT") })
        ...
        if cfg.BaseURL != "https://staging.it-help.tech" {
                t.Errorf("expected custom deployment base URL to be honored, got %s", cfg.BaseURL)
        }
}
