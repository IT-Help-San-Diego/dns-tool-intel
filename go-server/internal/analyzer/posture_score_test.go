...
func TestComputeInternalScore_Capped(t *testing.T) {
	...
	score, _ := computeInternalScore(ps, DKIMNoMailDomain)
	if score > 100 {
...
func TestComputeInternalScore_DKIMInconclusiveIsNeutral(t *testing.T) {
	...
	if got, _ := computeInternalScore(ps, DKIMInconclusive); got != 100 {
	...
	if got, _ := computeInternalScore(ps, DKIMAbsent); got != 85 {
...
func TestComputeInternalScore_IndeterminateNeutralized(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			ps := base
			tc.mutate(&ps)
			got, _ := computeInternalScore(ps, DKIMSuccess)
			if got != 100 {
