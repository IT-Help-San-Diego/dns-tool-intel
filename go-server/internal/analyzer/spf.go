...
// SPF presence tri-state (RFC 7208 §4.6: a "none" result — no SPF record — may
// only be returned from a completed query; a transient failure is "temperror",
// NOT "none"). mapKeySpfState lets drift and the UI tell "no record published"
// (absent_confirmed) apart from "lookup did not complete" (indeterminate),
// mirroring dane_state / dnssec_state.
const (
        mapKeySpfState        = "spf_state"
        spfStatePresent       = triStatePresent
        spfStateAbsentConf    = triStateAbsentConf
        spfStateIndeterminate = triStateIndeterminate
)
...
func (a *Analyzer) AnalyzeSPF(ctx context.Context, domain string) map[string]any {
        txtRecords, lookupStatus := a.resolveWithStatus(ctx, "TXT", domain)
        ...
        baseResult := map[string]any{
                ...
                "resolved_spf":         "",
                mapKeySpfState:         spfStateAbsentConf,
        }

        if len(txtRecords) == 0 {
                // RFC 7208 §4.6: only a completed query yields "none". A transient
