...
// TestAnalyzeSPF_WithMock_TXTPresentNoSPF locks the fix for the self-contradiction
// where a domain that publishes TXT records but no v=spf1 record reported
// status="missing" yet spf_state="present" (present was set unconditionally
// whenever any TXT record existed). The lookup completed and returned records, so
// the absence of SPF is authoritative — spf_state MUST read absent_confirmed and
// agree with the "missing" status.
func TestAnalyzeSPF_WithMock_TXTPresentNoSPF(t *testing.T) {
        a := newMockAnalyzer()
        mock := a.DNS.(*MockDNSClient)
        mock.AddResponse("TXT", "notspf.example.com", []string{
                "google-site-verification=abc123",
                "apple-domain-verification=XYZ",
        })

        ctx := context.Background()
        result := a.AnalyzeSPF(ctx, "notspf.example.com")

        if result["status"] != "missing" {
                t.Errorf("expected status 'missing', got %v", result["status"])
        }
        if got := result[mapKeySpfState]; got != spfStateAbsentConf {
                t.Errorf("expected spf_state %q (no contradiction with 'missing'), got %v", spfStateAbsentConf, got)
        }
}

// TestAnalyzeSPF_WithMock_ValidSPFStatePresent confirms the positive side: a real
// SPF record yields spf_state="present".
func TestAnalyzeSPF_WithMock_ValidSPFStatePresent(t *testing.T) {
        a := newMockAnalyzer()
        mock := a.DNS.(*MockDNSClient)
        mock.AddResponse("TXT", "haspf.example.com", []string{
                "google-site-verification=abc123",
                "v=spf1 include:_spf.apple.com ~all",
        })

        ctx := context.Background()
        result := a.AnalyzeSPF(ctx, "haspf.example.com")

        if got := result[mapKeySpfState]; got != spfStatePresent {
                t.Errorf("expected spf_state %q, got %v", spfStatePresent, got)
        }
}
