...
func TestExportBuildDNSVerdict(t *testing.T) {
        tests := []struct {
                ...
                {"dnssec validated", true, false, true},
                {"dnssec signed AD unset", true, false, false},
                {"dnssec broken", false, true, false},
                {"no dnssec", false, false, false},
        }
        for _, tt := range tests {
                t.Run(tt.name, func(t *testing.T) {
                        got := analyzer.ExportBuildDNSVerdict(tt.dnssecOK, tt.dnssecBroken, tt.dnssecADValidated)
                        if got == nil {
                                t.Error("expected non-nil map")
                        }
                })
