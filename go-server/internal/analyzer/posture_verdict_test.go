...
// TestBuildDNSVerdictDNSSECReason guards the claim-accuracy fix: the verdict must
// only assert resolver-confirmed validation when the AD flag was actually set, and
// must NOT claim the chain was "verified" when DNSKEY+DS are present but AD is unset.
func TestBuildDNSVerdictDNSSECReason(t *testing.T) {
        tests := []struct {
                name       string
                ps         protocolState
                wantSubstr string
