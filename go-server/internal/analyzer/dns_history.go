...
// dnsHistoryCacheKey returns a cache key scoped to both the domain and the
// caller's API key so that results fetched with one key are never served to a
// different caller.  The raw key is never stored; only a SHA-256 prefix is
// kept in the map entry.
func dnsHistoryCacheKey(domain, apiKey string) string {
	sum := sha256.Sum256([]byte(apiKey))
	return domain + "\x00" + hex.EncodeToString(sum[:16])
}
...
func FetchDNSHistoryWithKey(ctx context.Context, domain, userAPIKey string, cache *DNSHistoryCache) map[string]any {
	...
	cacheKey := dnsHistoryCacheKey(domain, userAPIKey)
