...
type AuthoritativeDSTTL struct {
	...
	Agreed   bool   `json:"agreed,omitempty"`    // a second parent NS was sampled and returned the same TTL
}
...
// cached it from any of them. So the result carries ParentNS (which server answered)
// and, when a second parent server can be resolved, it is queried too and Agreed is
// set only when both return the same TTL. A result with Present=true and Agreed=false
// is a single-server measurement, not a cross-checked one, and a consumer that quotes
// it as a client's outage window must say so.
//
...
func (a *Analyzer) queryAuthoritativeDSTTL(ctx context.Context, domain string) AuthoritativeDSTTL {
	...
	out := AuthoritativeDSTTL{TTL: *first.TTL, Present: true, ParentNS: parentIPs[0]}

	if len(parentIPs) > 1 {
		second := a.DNS.QueryWithTTLFromResolver(ctx, "DS", domain, parentIPs[1])
		if second.TTL != nil && *second.TTL == out.TTL {
			out.Agreed = true
		}
	}
