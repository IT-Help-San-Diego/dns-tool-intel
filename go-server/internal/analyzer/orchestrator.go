...
// tldSkippedProtocolKeys are the per-protocol result slots that do not apply
...
var tldSkippedProtocolKeys = []string{
	...
	mapKeyAiSurface, mapKeySecretExposure,
}

// applyTLDNotApplicable marks the email/subdomain protocol slots n/a for a
// registry-zone input. Named rather than inline so the behaviour is reachable
// from a test: it used to live inside computeSMTPResult, and when that
// function was removed the tests covering it referenced a symbol that no
// longer existed — a test file that cannot compile is the strongest form of a
...
// tldSMTPNotApplicable is the SMTP-transport result for a bare TLD. Absence of
// mail transport on a registry zone is the namespace, not a finding.
func tldSMTPNotApplicable() map[string]any {
