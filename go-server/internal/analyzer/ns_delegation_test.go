...
func TestClassifyEnterpriseDNS(t *testing.T) {
        tests := []struct {
                ...
                {
                        // Self-hosted NS whose name also matches a known provider
                        // (ns1.google.com under google.com) must be dedicated, not
                        // managed — self-hosting takes precedence over provider matching.
                        "self-hosted matching provider name",
                        "google.com",
