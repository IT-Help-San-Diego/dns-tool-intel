...
func TestClassifyDMARC_NoMailSuppressesRuaRecommendation(t *testing.T) {
	hasRec := func(recs []string, want string) bool {
		for _, r := range recs {
			if r == want {
				return true
			}
	...
	const visRec = "Add DMARC aggregate reporting (rua) for visibility into email authentication"
	const warnRec = "Enable DMARC aggregate reporting (rua) for authentication visibility"

	// No-mail domain (null MX, RFC 7505) with an enforcing DMARC policy and no
...
func TestClassifyDKIMPosture(t *testing.T) {
	tests := []struct {
		...
		// NOT absent: selectors are not enumerable from DNS, so failing to find
		// one at a guessed name is not evidence of absence. Carried as a
		// monitoring note instead, asserted below.
		{"inconclusive", DKIMInconclusive, "", false, false},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.ds == DKIMInconclusive {
				if len(acc.absent) != 0 {
					t.Errorf("inconclusive DKIM must not be counted as absent, got %v", acc.absent)
				}
				if len(acc.monitoring) == 0 {
					t.Error("inconclusive DKIM must surface as a monitoring note, not vanish")
				}
			}
		})
	}
