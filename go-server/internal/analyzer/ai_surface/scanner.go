...
var hiddenPatternRegexes = []struct {
	...
	{regexp.MustCompile(`clip-path\s*:\s*(inset\s*\(\s*100%|circle\s*\(\s*0)`), "CSS clip-path concealment"},
	{regexp.MustCompile(`clip\s*:\s*rect\s*\(\s*0[^)]{0,40}\)`), "CSS clip-rect concealment"},
}

// htmlCommentRe captures the body of an HTML comment. A comment is invisible
// to anyone reading the rendered page but fully present for any crawler or
// model ingesting the source — the same human/machine asymmetry as
// display:none, and therefore a natural hiding place for an injection
// payload. Until this was added the scanner could not see inside comments at
// all, so a payload parked there was invisible to the check designed to catch
// exactly that. Non-greedy so adjacent comments are not merged into one region.
var htmlCommentRe = regexp.MustCompile(`(?s)<!--(.*?)-->`)

// maxCommentScan bounds the region handed to the keyword search for a single
// comment; long licence headers and build banners are common and there is no
// value in scanning megabytes of them.
const maxCommentScan = 4000
...
func scanForHiddenPrompts(content string) []map[string]any {
	...
	// HTML comments are scanned as whole regions rather than via a window
	// after the match: the payload is the comment body itself.
	for _, m := range htmlCommentRe.FindAllStringSubmatchIndex(lower, -1) {
		body := lower[m[2]:m[3]]
		if len(body) > maxCommentScan {
			body = body[:maxCommentScan]
		}
		artifacts = append(artifacts, findKeywordsInRegion(body, "HTML comment", seen)...)
	}
