...
// newSSRFSafeClient returns an HTTP client hardened against SSRF:
//   - Custom DialContext validates the resolved IP at TCP-dial time (blocks DNS rebinding).
//   - CheckRedirect refuses all redirects (blocks redirect-based SSRF).
func newSSRFSafeClient() *http.Client {
	...
	transport := &http.Transport{
		DialContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
			return ssrfSafeDialContext(ctx, baseDialer, network, addr)
		},
	}
	return &http.Client{
		Timeout:   httpTimeout,
		...
		CheckRedirect: func(_ *http.Request, _ []*http.Request) error {
			return fmt.Errorf("webhook redirects are not permitted")
		},
	}
}

// ssrfSafeDialContext resolves the target hostname to IPs at dial time, validates
// every returned address against the private/loopback/link-local block list, and
// then dials the first clean IP directly.  Dialing the IP directly (rather than
// the hostname) prevents a second OS-level DNS resolution from returning a
// rebinded private address after the pre-flight isSSRFSafe check has passed.
func ssrfSafeDialContext(ctx context.Context, dialer *net.Dialer, network, addr string) (net.Conn, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("dial-time DNS lookup failed for %q: %w", host, err)
	}
	if len(ips) == 0 {
	...
	for _, ipStr := range ips {
		...
		if isRestrictedIP(ip) {
			return nil, fmt.Errorf("blocked: %q resolved to restricted IP %s at dial time", host, ipStr)
		}
	}

	// Connect to the first validated IP directly so no further DNS resolution occurs.
	return dialer.DialContext(ctx, network, net.JoinHostPort(ips[0], port))
}

// isRestrictedIP reports whether ip must never be a webhook target.
...
// isSSRFSafe performs a pre-flight hostname resolution and rejects any URL that
// resolves to a private, loopback, or link-local address.  This is a defense-in-
// depth layer; the primary guard against DNS rebinding is the dial-time check in
// ssrfSafeDialContext (used by the client returned from newSSRFSafeClient).
...
// ValidateWebhookURL is the public entry point for SSRF pre-flight validation of
// a user-supplied webhook URL.  Callers that store URLs (e.g. AddEndpoint) should
// call this before persisting so users receive an early rejection.
func ValidateWebhookURL(rawURL string) error {
	return isSSRFSafe(rawURL)
}
