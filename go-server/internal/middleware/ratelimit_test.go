...
// --- AllowKey per-scan charging invariants (batch key bucket) ---

func TestAllowKeyOversizeSingleCallRefusesPermanentlyWithoutPanic(t *testing.T) {
	limiter := NewInMemoryRateLimiter()
	// Fresh (empty) window + scans > cap: the pre-guard must refuse with
	// retry -1 (a call that can NEVER fit gets no retry advice). Before the
	// guard existed this exact shape indexed keep[0] on an empty slice.
	ok, retry := limiter.AllowKey(9, KeyScanCapPerMinute+1)
	if ok {
		t.Fatal("scans > cap must refuse")
	}
	if retry != -1 {
		t.Fatalf("oversize refusal must be permanent (retry -1), got %d", retry)
	}
}

func TestAllowKeyCapExactFitsThenRefusesRetryable(t *testing.T) {
	limiter := NewInMemoryRateLimiter()
	if ok, _ := limiter.AllowKey(1, KeyScanCapPerMinute); !ok {
		t.Fatal("a full-cap charge must fit an empty window")
	}
	ok, retry := limiter.AllowKey(1, 1)
	if ok {
		t.Fatal("window full: one more scan must refuse")
	}
	if retry < 1 || retry > 61 {
		t.Fatalf("retryable refusal must advise 1..61s, got %d", retry)
	}
}

func TestAllowKeyExpiredEntriesFreeTheWindow(t *testing.T) {
	limiter := NewInMemoryRateLimiter()
	old := float64(time.Now().Add(-2 * time.Minute).Unix())
	limiter.mu.Lock()
	limiter.requests["scankey:3"] = []requestEntry{{timestamp: old}, {timestamp: old}}
	limiter.mu.Unlock()
	if ok, _ := limiter.AllowKey(3, KeyScanCapPerMinute); !ok {
		t.Fatal("entries older than the window must not count against the cap")
	}
}

func TestAllowKeyZeroScansFreeAndNonConsuming(t *testing.T) {
	limiter := NewInMemoryRateLimiter()
	if ok, retry := limiter.AllowKey(4, 0); !ok || retry != 0 {
		t.Fatal("zero scans must be allowed and free")
	}
	if ok, _ := limiter.AllowKey(4, KeyScanCapPerMinute); !ok {
		t.Fatal("zero-scan call must not have consumed budget")
	}
}
