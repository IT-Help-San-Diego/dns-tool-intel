...
type DBTX interface {
	...
	ICuAEGetGradeDistributionCutoff(ctx context.Context) (time.Time, error)
...
func LoadRuntimeMetrics(ctx context.Context, queries DBTX) *RuntimeMetrics {
	...
	m.GradeDist, m.GradeDistCutoff, _ = loadGradeDistribution(ctx, queries)
...
// loadGradeDistribution returns the grade distribution scoped to rows recorded
// at/after the 018 grade-width adoption, plus the cutoff timestamp it was
// scoped to (rendered in the caption — label and filter from one source).
// applied is false when 018 is not yet applied (cutoff query returns no row):
// the caller renders no distribution in that case, never an unfiltered one.
func loadGradeDistribution(ctx context.Context, queries DBTX) (items []GradeDistItem, cutoff string, applied bool) {
	cutoffTs, err := queries.ICuAEGetGradeDistributionCutoff(ctx)
	if err != nil {
		// 018 not yet applied (pgx.ErrNoRows) or ledger unreadable.
		return nil, "", false
	}
	applied = true
	cutoff = cutoffTs.Format("2006-01-02")
