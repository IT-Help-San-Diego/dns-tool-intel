...
func TestHistoryPerPageClampsClientValue(t *testing.T) {
	...
	for _, tc := range cases {
		...
		if tc.cookie != "" {
			c.Request.AddCookie(&http.Cookie{Name: historyRowsCookie, Value: tc.cookie})
		}
		if got := historyPerPage(c); got != tc.want {
			t.Errorf("historyPerPage(%q) = %d, want %d", tc.cookie, got, tc.want)
		}
	}
}
