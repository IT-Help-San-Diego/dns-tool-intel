...
func (h *ZoneHandler) UploadForm(c *gin.Context) {
        data := NewTemplateData(c, h.Config, "zone")
...
func (h *ZoneHandler) ProcessUpload(c *gin.Context) {
        ...
        if err != nil {
                h.renderZoneFlash(c, mapKeyDanger, "Please select a zone file to upload.")
                return
        }
        defer safeClose(file, "zone file")

        if header.Size > maxSize {
                h.renderZoneFlash(c, mapKeyDanger, fmt.Sprintf("Zone file exceeds the %s size limit.", sizeLabel))
                return
        }
        ...
        if err != nil {
                ...
                h.renderZoneFlash(c, mapKeyDanger, "Failed to parse zone file: "+err.Error())
                return
        }

        if parseResult.RecordCount == 0 {
                h.renderZoneFlash(c, "warning", "No DNS records found in the uploaded zone file. Verify the file format.")
                return
        }

        domain := parseResult.Domain
        if domain == "" {
                h.renderZoneFlash(c, mapKeyDanger, "Could not determine the domain from the zone file. Please provide a domain override.")
                return
        }
        ...
        data["ZoneHealth"] = zoneHealth
        data["DriftReport"] = driftReport
        data["LiveAnalysisID"] = liveAnalysisID
...
func (h *ZoneHandler) renderZoneFlash(c *gin.Context, category, message string) {
        data := NewTemplateData(c, h.Config, "zone")
        data[strShowform] = true
