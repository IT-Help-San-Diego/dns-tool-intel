...
func NewAuthHandlerWithStore(cfg *config.Config, pool *pgxpool.Pool, store AuthStore) *AuthHandler {
        h := &AuthHandler{
                Config:    cfg,
                Pool:      pool,
                authStore: store,
        ...
        if pool != nil {
                h.Queries = dbq.New(pool)
        }
        return h
}

func (h *AuthHandler) SetStore(s AuthStore) {
        h.authStore = s
}
