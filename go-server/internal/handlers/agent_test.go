...
func TestBuildAgentHTMLZoteroMetadata(t *testing.T) {
        ...
        zoteroChecks := []string{
                ...
                `name="citation_doi" content="10.5281/zenodo.19468134"`,
                `class="Z3988"`,
                `ctx_ver=Z39.88-2004`,
                `property="og:title"`,
                `property="og:image"`,
...
func TestWrapperInlineStylesAndLinks(t *testing.T) {
        router, h := setupAgentRouter()
        router.GET("/agent/snapshot-view", h.SnapshotView)
        router.GET("/agent/json-view", h.JSONView)
        router.GET("/agent/csv-view", h.CSVView)
        router.GET("/agent/checksum-view", h.ChecksumView)
        router.GET("/agent/wayback-view", h.WaybackHTMLView)
        ...
        tests := []struct {
                ...
                {
                        "csv-view has inline styles, no report link, centered CTA",
                        "/agent/csv-view?domain=example.com",
                        []string{
                                `style="` + iwBody + `"`,
                                `style="` + iwCard + `"`,
                ...
                {
                        "wayback-view has inline styles, no report link, no secondary btn",
                        "/agent/wayback-view?domain=example.com",
                        []string{
                                `style="` + iwBody + `"`,
                                `style="` + iwCard + `"`,
                ...
                {
                        "snapshot-view has inline styles, keeps secondary btn, no meta report link",
                        "/agent/snapshot-view?domain=example.com",
                        []string{
                                `style="` + iwBody + `"`,
                                `style="` + iwDlBtn + `"`,
                ...
                {
                        "json-view has inline styles, keeps secondary btn, no meta report link",
                        "/agent/json-view?domain=example.com",
                        []string{
                                `style="` + iwBody + `"`,
                                "View JSON Data",
                ...
                {
                        "checksum-view has inline styles, keeps secondary btns, no meta report link",
                        "/agent/checksum-view?domain=example.com",
                        []string{
                                `style="` + iwBody + `"`,
                                "View Checksum (JSON)",
                ...
                t.Run(tt.name, func(t *testing.T) {
                        ...
                        for _, p := range tt.present {
                                if !strings.Contains(body, p) {
                                        t.Errorf("expected to find %q in response", p)
                                }
                        }
                        for _, a := range tt.absent {
                                if strings.Contains(body, a) {
                                        t.Errorf("expected %q to be absent from response", a)
                                }
                        }
                })
        }
