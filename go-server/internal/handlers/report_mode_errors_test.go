...
func TestComputeSummary(t *testing.T) {
	...
	t.Run("with data", func(t *testing.T) {
		...
		if s.TotalUniqueVisitors != 0 {
			t.Errorf("TotalUniqueVisitors = %d, want 0 (no HLL without DB)", s.TotalUniqueVisitors)
		}
		...
		if s.AvgDailyVisitors != 0 {
			t.Errorf("AvgDailyVisitors = %d, want 0 (no HLL without DB)", s.AvgDailyVisitors)
		}
