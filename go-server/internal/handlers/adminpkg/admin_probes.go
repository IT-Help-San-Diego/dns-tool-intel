...
func NewProbeAdminHandler(database *db.Database, cfg *config.Config, tdf TemplateDataFunc) *ProbeAdminHandler {
        return &ProbeAdminHandler{DB: database, Config: cfg, TemplateData: tdf}
}
...
func (h *ProbeAdminHandler) ProbeDashboard(c *gin.Context) {
        ...
        data := h.TemplateData(c, h.Config, "admin")
        data["Probes"] = probes
...
func safeClose(c io.Closer, label string) {
        if err := c.Close(); err != nil {
                slog.Warn("close error", "label", label, "error", err)
        }
}
...
type sshTarget struct {
        ...
        user    string
        signer  ssh.Signer
        hostKey ssh.PublicKey // optional pinned host key; nil → fallback with warning
}

// parseHostKeyEnv parses an optional host key in OpenSSH authorized_keys
// format (e.g. "ssh-ed25519 AAAA..."). Empty string returns (nil, nil).
func parseHostKeyEnv(s string) (ssh.PublicKey, error) {
...
func resolveProbeSSH(probeID string) (*sshTarget, error) {
        ...
        case strProbe02:
                ...
                hostKeyEnv = os.Getenv("PROBE_SSH_HOST_KEY_2")
                if host == "" || user == "" || keyB64 == "" {
        ...
        hostKey, err := parseHostKeyEnv(hostKeyEnv)
        if err != nil {
                return nil, fmt.Errorf("%s: %w", probeID, err)
        }
        ...
        return &sshTarget{host: host, user: user, signer: signer, hostKey: hostKey}, nil
}
...
func executeSSH(ctx context.Context, target *sshTarget, script string) (string, error) {
        ...
        if target.hostKey != nil {
                hostKeyCallback = ssh.FixedHostKey(target.hostKey)
        } else {
                slog.Warn("SSH probe host key not pinned — set PROBE_SSH_HOST_KEY[_2] to enable verification",
                        "host", target.host)
                hostKeyCallback = ssh.InsecureIgnoreHostKey() //nolint:gosec // backward-compatible fallback when host key env unset
        }
