...
func (h *FailuresHandler) Failures(c *gin.Context) {
        ...
        if err != nil {
                errData := NewTemplateData(c, h.Config, "failures")
                errData["FlashMessages"] = []FlashMessage{{Category: "danger", Message: "Failed to fetch failure log"}}
        ...
        data := NewTemplateData(c, h.Config, "failures")
        data["Failures"] = entries
        data["TotalFailed"] = statsFailedCount
        data["TotalAnalyses"] = totalAll
