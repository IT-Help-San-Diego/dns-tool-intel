...
type WebhookSender func(ctx context.Context, webhookURL string) error

type WatchlistHandler struct {
        DB              *db.Database
        Config          *config.Config
        Notifier        *notifier.Notifier
        TestWebhookFunc WebhookSender
}
...
func (h *WatchlistHandler) TestWebhook(c *gin.Context) {
        ...
        ctx := c.Request.Context()
        sender := WebhookSender(h.Notifier.SendTestDiscord)
        if h.TestWebhookFunc != nil {
                sender = h.TestWebhookFunc
        }
        if err := sender(ctx, h.Config.DiscordWebhookURL); err != nil {
