...
func TestExtractOAuthCallbackParams_SecurityBoundary(t *testing.T) {
        ...
        t.Run("state mismatch rejects callback — CSRF prevention", func(t *testing.T) {
                ...
                c.Request.AddCookie(&http.Cookie{Name: authpkg.OauthStateCookie, Value: "legitimate_state"})
                c.Request.AddCookie(&http.Cookie{Name: authpkg.OauthCVCookie, Value: "verifier"})
                c.Request.AddCookie(&http.Cookie{Name: authpkg.OauthNonceCookie, Value: "nonce"})
        ...
        t.Run("missing code verifier rejects — PKCE enforcement", func(t *testing.T) {
                ...
                c.Request.AddCookie(&http.Cookie{Name: authpkg.OauthStateCookie, Value: "abc"})
                c.Request.AddCookie(&http.Cookie{Name: authpkg.OauthNonceCookie, Value: "nonce"})
        ...
        t.Run("valid callback returns all parameters", func(t *testing.T) {
                ...
                c.Request.AddCookie(&http.Cookie{Name: authpkg.OauthStateCookie, Value: "secure_state"})
                c.Request.AddCookie(&http.Cookie{Name: authpkg.OauthCVCookie, Value: "pkce_verifier"})
                c.Request.AddCookie(&http.Cookie{Name: authpkg.OauthNonceCookie, Value: "replay_nonce"})

                state, verifier, nonce, code, ok := authpkg.ExtractOAuthCallbackParams(c)
                if !ok {
...
func TestExtractUserClaims_EdgeCases(t *testing.T) {
        t.Run("numeric sub is handled gracefully", func(t *testing.T) {
                sub, _, _, _ := authpkg.ExtractUserClaims(map[string]any{"sub": 12345})
                if sub != "" {
        ...
        t.Run("nil email_verified returns false", func(t *testing.T) {
                _, _, _, verified := authpkg.ExtractUserClaims(map[string]any{"email_verified": nil})
                if verified {
        ...
        t.Run("string 'true' for email_verified is treated as false — type safety", func(t *testing.T) {
                _, _, _, verified := authpkg.ExtractUserClaims(map[string]any{"email_verified": "true"})
                if verified {
...
func TestAuthCookieSecurityProperties(t *testing.T) {
        if authpkg.SessionCookieName != "_dns_session" {
                t.Errorf("session cookie name = %q, expected '_dns_session'", authpkg.SessionCookieName)
        }
        if authpkg.SessionMaxAge != 30*24*60*60 {
                t.Errorf("session max age = %d seconds, expected 30 days (%d)", authpkg.SessionMaxAge, 30*24*60*60)
        }
        ...
        if authpkg.OauthHTTPTimeout != 10*time.Second {
                t.Errorf("OAuth HTTP timeout = %v, expected 10s", authpkg.OauthHTTPTimeout)
        }
        if authpkg.IatMaxSkew != 5*time.Minute {
                t.Errorf("iat max skew = %v, expected 5 minutes", authpkg.IatMaxSkew)
        }
        t.Logf("MEASUREMENT: OAuth timeout=%v iat_skew=%v — balances security vs clock drift", authpkg.OauthHTTPTimeout, authpkg.IatMaxSkew)
}

func TestGoogleOAuthEndpoints_RFC8414(t *testing.T) {
        if authpkg.GoogleAuthURL != "https://accounts.google.com/o/oauth2/v2/auth" {
                t.Errorf("auth endpoint = %q — must match Google OIDC discovery", authpkg.GoogleAuthURL)
        }
