...
func NewAgentHandler(cfg *config.Config, a *analyzer.Analyzer, tdf TemplateDataFunc, store ...LookupStore) *AgentHandler {
        h := &AgentHandler{Config: cfg, Analyzer: a, TemplateData: tdf}
        if len(store) > 0 {
...
func (h *AgentHandler) buildAgentJSON(domain string, results map[string]any) gin.H {
        ...
        return gin.H{
                ...
                "provenance": gin.H{
                        ...
                        "methodology":    "RFC-grounded analysis with " + confidenceMethodology,
...
<head>
  <meta charset="UTF-8">
  <title>` + esc(strings.Title(styleName)) + ` Security Badge — ` + ed + `</title>
  <meta name="description" content="` + es + ` DNS security posture badge for ` + ed + ` — generated by DNS Tool.">
  <meta name="robots" content="noindex">
  <meta property="og:title" content="` + esc(strings.Title(styleName)) + ` Security Badge — ` + ed + `">
