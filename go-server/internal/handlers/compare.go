...
func renderCompareError(c *gin.Context, p compareErrorParams) {
        data := NewTemplateData(c, p.handler.Config, mapKeyCompare)
        data[strFlashmessages] = []FlashMessage{{Category: "danger", Message: p.message}}
        if p.domain != "" {
...
func (h *CompareHandler) Compare(c *gin.Context) {
        ...
        if err != nil {
                renderCompareError(c, compareErrorParams{handler: h, tmpl: templateCompare, statusCode: http.StatusNotFound, message: "Analysis A not found"})
                return
        }
        ...
        if analysisA.Domain != analysisB.Domain {
                renderCompareError(c, compareErrorParams{handler: h, tmpl: templateCompareSelect, statusCode: http.StatusBadRequest, message: "Cannot compare analyses of different domains", domain: analysisA.Domain})
                return
        }
        ...
        if resultsA == nil || resultsB == nil {
                renderCompareError(c, compareErrorParams{handler: h, tmpl: templateCompareSelect, statusCode: http.StatusBadRequest, message: "One or both analyses have no stored data", domain: analysisA.Domain})
                return
        }
        ...
        data := NewTemplateData(c, h.Config, mapKeyCompare)
        data[strDomain] = analysisA.Domain
        data["AnalysisA"] = buildCompareAnalysis(analysisA)
        data["AnalysisB"] = buildCompareAnalysis(analysisB)
        data["Diffs"] = diffItems
        data["ChangesFound"] = changesFound
        c.HTML(http.StatusOK, templateCompare, data)
}

func (h *CompareHandler) selectDomain(c *gin.Context, domain string) {
        if domain == "" {
                emptyData := NewTemplateData(c, h.Config, mapKeyCompare)
                emptyData[strDomain] = ""
                emptyData[strFlashmessages] = []FlashMessage{{Category: "warning", Message: "Please provide a domain to compare analyses."}}
        ...
        if err != nil {
                fetchErrData := NewTemplateData(c, h.Config, mapKeyCompare)
                fetchErrData[strDomain] = domain
                fetchErrData[strFlashmessages] = []FlashMessage{{Category: "danger", Message: "Failed to fetch analyses"}}
        ...
        selectData := NewTemplateData(c, h.Config, "")
        selectData[strDomain] = domain
        selectData["Analyses"] = items
