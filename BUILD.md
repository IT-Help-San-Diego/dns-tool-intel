...
Point it at an **empty** database and it creates the schema itself on first
start. Point it at an existing one and it upgrades it. Both go through the same
versioned migration chain (see *Database requirements* below), so there is no
separate schema-loading step.
...
**Schema is versioned and self-applying.** The migrations in
`go-server/db/migrations/` are compiled into the binary (`go:embed`) and applied
at startup: only pending versions run, in order, each in its own transaction.
The applied set is recorded in `goose_db_version`, with file checksums in
`schema_migration_checksums`.

What this means in practice:

- **Empty database** → the full schema is created. No manual step, and no
  requirement to start the server from the repository root.
- **Existing database** → only the migrations it has not seen are applied. Your
  data is not touched. A database that predates the version ledger is *adopted*:
  the runner records which migrations its schema already reflects and executes
  nothing against it.
- **Database newer than the binary** → the server refuses to start and names
  both versions. Running old code against a new schema produces wrong reads
  rather than visible failures, so this is deliberate.
- **An applied migration was edited** → the server refuses to start. The
  database no longer contains what the chain says it contains. Restore the file
  and express the change as a new migration.

Any failure in this path is fatal: the server exits non-zero rather than serving
from a database whose shape it cannot vouch for.

`go-server/db/schema/schema.sql` is **documentation only** — `pg_dump
--schema-only` of the migrated schema, regenerated by
`./scripts/regen-schema-doc.sh`. Nothing executes it.
