...
> *Every finding in this codebase carries a fingerprint, a severity, and a public record.*
> *An institution that publishes its failures is making a checkable claim about its own reliability.*

**Registry opened**: 2026-03-18 — originally published as "Failure Registry Reviews" and renamed
2026-07-31. The findings, their fingerprints, the five-tier severity scale with S0 for regressions,
and the T001-T005 team designations are unchanged; only the vocabulary was retired. The reasoning is
recorded in `docs/evolution/2026-07-31-failure-registry-rename.md` and migration 017 — this document
is the current record, not the historical one.
...
**Review Teams**: 5 parallel strike teams deployed simultaneously
...
Every finding is classified by the damage they inflict on the system and its users.
...
| Status | Meaning |
|--------|---------|
| `OPEN` | Identified. Recorded. Awaiting analysis. |
| `UNDER ANALYSIS` | Actively being investigated or fixed. |
| `RESOLVED` | Eliminated. Fix deployed. Commit hash recorded. |
| `REFERRED` | Transferred — out of scope or owned by upstream dependency. |
...
- **Analysis Notes**: On the Engineer's Report, pressing the covert button doesn't toggle — it navigates to `/analysis/{id}/view/C` (full page load). Pressing it again navigates back to `/view/E`. User loses scroll position, loading state resets, and the interaction feels fundamentally broken. On non-results pages, the same button does a client-side class toggle. The inconsistency is the real crime — two completely different behaviors behind the same red button.
...
- **Status**: `RESOLVED` — scroll position restored via sessionStorage on page navigation
...
- **Status**: `RESOLVED` — `.card.bg-dark` already had glass/hover; added `border-accent-gold-muted` CSS class and `.card.border-primary.bg-primary.bg-opacity-10` glass treatment
...
- **Status**: `RESOLVED` — `@media (prefers-reduced-motion: reduce)` block added at end of custom.css
...
- **Status**: `RESOLVED` — color changed to `#8b949e` (~6.3:1 contrast on `#0d1117`)
...
- **Status**: `RESOLVED` — `tabindex="0"`, `role="button"`, `aria-label` added to compare rows; keyboard event handlers present
...
- **Analysis Notes**: Two SVG diagrams return 404: `drift-notification-pipeline.svg` and `github-issues-triage.svg`. These are on the architecture page — the page specifically designed to demonstrate our engineering discipline and system design rigor.
...
- **Status**: `RESOLVED` — SVG files confirmed present in `/static/images/diagrams/`; 404 was transient or from stale cache
...
- **Analysis Notes**: `.copy-btn` has `opacity: 0` by default and only becomes visible on `:hover`. This means: (1) keyboard-only users never see the button, (2) keyboard-only users cannot focus on an invisible element in most implementations, (3) screen readers may or may not announce an opacity-0 element depending on browser. The functionality exists but is gated behind mouse ownership.
...
- **Status**: `RESOLVED` — `:focus-visible` added alongside `:hover`/`:focus` with outline for keyboard visibility
...
- **Analysis Notes**: A generic smooth-scroll handler is attached to ALL `a[href^="#"]` links. It calls `e.preventDefault()` on every hash link click, then smooth-scrolls to the target. Problem: Bootstrap collapse triggers that use `href="#target"` (instead of `data-bs-target`) get their default behavior blocked. The collapse never fires. The section never opens. No error. No feedback. Silent failure.
...
- **Status**: `RESOLVED` — smooth-scroll handler now checks for `data-bs-toggle` attribute before calling `preventDefault()`
...
- **Analysis Notes**: Template uses CSS class `white-space-nowrap`. This class does not exist in Bootstrap or our custom CSS. The intended class is Bootstrap's `text-nowrap`. As-is, the class does nothing — the button text wraps freely, especially on mobile where it shouldn't.
- **Status**: `RESOLVED` — replaced `white-space-nowrap` with Bootstrap `text-nowrap`
...
- **Analysis Notes**: The Currency card references `border-accent-gold-muted` as a CSS class. grep confirms: no CSS rule exists for this class anywhere in the codebase. The border color falls back to the default, making the Currency card's border visually identical to an unstyled card. The gold accent that was clearly intended is absent.
...
- **Status**: `RESOLVED` — `.border-accent-gold-muted` CSS class defined with gold border color and hover treatment
...
- **Status**: `RESOLVED` — investigation confirmed no write-capable tokens exposed; OAuth client ID is semi-public by design per OAuth 2.0 spec; no Sanity Token found in page source
...
- **Analysis Notes**: The `script-src` directive uses `*`, allowing JavaScript execution from any origin. This effectively disables one of the strongest browser-side defenses against XSS. We audit DNS security configurations for domains around the world, and our own CSP is wide open.
- **Damage Assessment**: Security credibility risk. Anyone who inspects our headers will see the contradiction.
- **Status**: `RESOLVED` — CSP already uses nonce-based `script-src 'self' 'nonce-{n}'`; wildcard `*` was not present in production middleware
...
- **Analysis Notes**: In the Domain Summary section, Registrar, Email Hosting, and Web Hosting columns all use `text-truncate`. The DNS Hosting column does not. Long provider names overflow their container, especially on mobile at 375px.
- **Status**: `OPEN`
...
- **Analysis Notes**: Bootstrap tooltip initialization happens only in an inline script inside `results.html`. The global `main.js` has zero tooltip initialization. Any Bootstrap tooltips on comparison, history, stats, or other pages silently do nothing. The `data-bs-toggle="tooltip"` attribute is there. The JavaScript to activate it is not.
- **Status**: `OPEN`
...
- **Analysis Notes**: Navbar collapse dropdown uses `position: absolute` with no `max-height` and no `overflow-y: auto`. With 11+ nav items plus potential auth dropdown items, on shorter mobile screens the menu extends below the viewport. The user cannot scroll to see the bottom nav items.
- **Status**: `OPEN`
...
- **Analysis Notes**: Ten or more footer links crammed into a single `<p>` tag, separated by `&middot;` dots. At 375px mobile, this renders as an unstructured wall of text with no grouping, no columns, no visual hierarchy. Finding any specific link requires reading the entire paragraph.
- **Status**: `OPEN`
...
- **Threat Level**: EXPLOIT
- **Captured By**: T001 (SquirrelScan)
- **Analysis Notes**: The video category scored 57 out of 100 — an F. Likely missing VideoObject structured data, accessibility attributes (`track` elements for captions), poster images, or proper `preload` configuration. This drags the overall site score down significantly.
- **Status**: `OPEN` — needs targeted investigation
...
- **Analysis Notes**: ~450 in covert mode alone. Architecturally inevitable without CSS `@layer`. Every future style addition must also use `!important` or lose. The specificity ceiling gets lower every sprint.
...
- **Analysis Notes**: We have a documented 5-layer color architecture with CSS custom properties. And then 50+ hex values that bypass it entirely. Any theme change requires hunting through 7,258 lines of CSS for rogue colors.
- **Status**: `OPEN`
...
- **Threat Level**: CVE
- **Captured By**: T004 (Design Forensics)
- **Analysis Notes**: `backdrop-filter` uses 12px, 8px, 6px, 4px across different components with no documented rationale. No `--blur-*` design tokens. Should be: `--blur-heavy: 12px`, `--blur-medium: 8px`, `--blur-subtle: 4px`.
- **Status**: `OPEN`
...
- **Threat Level**: CVE
- **Captured By**: T004 (Design Forensics)
- **Analysis Notes**: `0.15s`, `0.2s`, `0.25s`, `0.3s`, `0.35s` — five different transition speeds across interactive elements. No `--transition-fast`, `--transition-normal`, `--transition-slow` tokens. Each developer picked what "felt right."
- **Status**: `OPEN`
...
- **Analysis Notes**: `transition: all 0.2s ease` animates every CSS property that changes — including `width`, `height`, `box-shadow`, and layout-triggering properties. Should specify exact properties: `transition: transform 0.2s ease, box-shadow 0.2s ease`.
- **Status**: `OPEN`
...
- **Threat Level**: CVE
- **Captured By**: T004 (Design Forensics)
- **Analysis Notes**: Mixes `max-width: 767px`, `max-width: 767.98px`, and `max-width: 768px` — three different expressions for the same mobile/tablet boundary. Bootstrap convention is `767.98px`. The others create 1-2px zones where styles may overlap or gap.
- **Status**: `OPEN`
...
- **Threat Level**: CVE
- **Captured By**: T004 (Design Forensics)
- **Analysis Notes**: Dedicated `print.css` file, PLUS 3 separate `@media print` blocks in `custom.css` (lines 5731, 6706, 6791), PLUS inline `<style>` in `results_executive.html`. Four sources of truth for print styling. When one changes, the others drift.
- **Status**: `OPEN`
...
- **Threat Level**: CVE
- **Captured By**: T004 (Design Forensics)
- **Analysis Notes**: `.code-block` has covert mode overrides. `.u-code-block` does not — bright syntax colors survive into covert mode. The ICAE card with its gold gradient border has no covert override either — it visually pops against the dimmed red background like a lighthouse.
- **Status**: `OPEN`
...
- **Threat Level**: CVE
- **Captured By**: T004 (Design Forensics)
- **Analysis Notes**: Non-print CSS should standardize on `rem` for accessibility (respects user font-size preferences). Currently mixes `rem`, `px`, and `pt`. The `px` and `pt` values will not scale with user preference changes.
- **Status**: `OPEN`
...
- **Threat Level**: CVE
- **Captured By**: T004 (Design Forensics)
- **Analysis Notes**: `.code-block` (line 999) has copy button, hover effects, covert mode overrides. `.u-code-block` (line 5299) has none of those. Both exist. Templates may use either. There's no documented reason for two implementations.
- **Status**: `OPEN`
...
- **Threat Level**: CVE
- **Captured By**: T001 (SquirrelScan)
- **Analysis Notes**: `/topology`, `/compare`, and 66+ dynamic pages lack `<link rel="canonical">`. Search engines may index multiple URLs for the same content, diluting page authority.
- **Status**: `OPEN`
...
- **Threat Level**: CVE
- **Captured By**: T001 (SquirrelScan)
- **Analysis Notes**: `<meta charset="UTF-8">` must be within the first 1024 bytes of the document and should be the first child of `<head>`. Currently it's not first. Browsers may misinterpret encoding for content that appears before the charset declaration.
- **Status**: `OPEN`
...
- **Threat Level**: CVE
- **Captured By**: T001 (SquirrelScan)
- **Analysis Notes**: Four external links broken: two DTIC military research PDFs (403 — access restricted), IANA RDAP reference (404), DNI ICD 203 PDF (404). These are cited as authoritative sources on the homepage and approach page.
- **Status**: `OPEN`
...
- **Threat Level**: CVE
- **Captured By**: T001 (SquirrelScan)
- **Analysis Notes**: History pagination pages share identical titles. Analysis view and analyze pages share titles. Search engines treat duplicate titles as duplicate content signals.
- **Status**: `OPEN`
...
- **Threat Level**: IOC
- **Captured By**: T004 (Design Forensics)
- **Analysis Notes**: Browsers that don't support `backdrop-filter` render semi-transparent backgrounds without blur. Text over complex backgrounds becomes unreadable.
- **Status**: `OPEN`
...
*When a finding is eliminated, we record the kill. Date, commit hash, who pulled the trigger. We don't forget what we killed — and we don't let them come back.*

| Hash | BSI | Resolved | Commit | Operative |
|------|-----|----------|--------|-----------|
...
> *They enter as OPEN. They leave as RESOLVED — or they don't leave at all.*
