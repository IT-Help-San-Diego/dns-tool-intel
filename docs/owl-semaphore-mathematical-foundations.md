...
4. **The Brier score connection**: DNS Tool's reliability weighting uses Brier scores. The Brier score B = (1/N) Σ(f_i − o_i)² measures the mean squared error between forecast probabilities and observed outcomes. Is there a natural group-theoretic interpretation of the Brier score in terms of the O(2) rotation? Specifically: does a well-calibrated system (B → 0) correspond to a system whose epistemic states cluster near the identity and whose angular excursions from I are well-predicted?
