...
- **Version Law — version comes from git, not a hand-edited file** (changed 2026-06-20; replaces the old Two-Track dev-bump file churn that conflicted on every ship):
  ...
  - **Release** (tag time only): Run `bash scripts/release-gate.sh X.Y.Z` (bumps the Zenodo/CITATION artifacts + PDFs), then create an annotated git tag **on `origin/main`'s tip** and push it: `git fetch origin main && git tag -a vX.Y.Z origin/main -m "Release vX.Y.Z" && git push origin vX.Y.Z`. The tag IS the version — `git describe` then resolves to exactly `X.Y.Z` for builds from `main` (i.e. production). Only when Carey is ready to tag. **CRITICAL — tag `origin/main`, NOT local HEAD**: because the ship flow squash-merges, the local `replit-agent` HEAD is NEVER on `main`'s lineage (it diverges by every pre-squash commit). A tag placed on local HEAD makes `git describe` read clean locally but is UNREACHABLE from `main`, so production keeps reading the previous tag. Verify after tagging: `gh api repos/<org>/<repo>/compare/main...vX.Y.Z --jq '.status'` must be `identical` or `behind` (NOT `diverged`/`ahead`). If a tag was mis-placed on local HEAD, fix it: `git push origin :refs/tags/vX.Y.Z` (delete remote) → `git tag -d vX.Y.Z` (delete local) → re-tag on `origin/main` as above.
...
- **THE THREE-COMMAND PLANS** (canonical sequences — copy to notes):
  ...
  **A. Ship a dev bump to `origin/main`** (post-branch-protection 2026-05-16):
  ...
  ```
  `git-push.sh` is also self-guarding against the duplicate-ship failure mode: before opening a PR it compares the local HEAD git-tree SHA to `origin/main`'s tree and HARD-STOPS with "Nothing to ship" if they are identical (re-running after a merge without syncing used to open back-to-back no-op PRs that still squash-merged — e.g. #124/#125/#126). Its merge-wait loop breaks on `state==MERGED` (not just `merged==true`), bails immediately if the PR is `CLOSED`, and now also bails in ~40s if the PR is `mergeable==CONFLICTING` (two consecutive reads, to ride out GitHub's brief `UNKNOWN` mergeability window) — auto-merge can never fire on a conflicting PR, so it no longer burns the full 15-min timeout (the chronic "stuck on git push" hang); it prints the `sync-local-to-main.sh` remedy and exits. So a transient `gh` read can no longer leave it polling to the 15-min timeout.
  `git-push.sh` handles the PR lifecycle automatically. No direct push to main. No API ref-writes. Three permanent design pillars (all 2026-06-08):
  - **Sync after every ship (step 4)**: squash-merges land on `main` as commits the local branch never receives, so local + main share only an ancient ancestor. Because every release re-bumps the same `Version` line (`config.go` + `sonar-project.properties`), a 3-way merge sees that line "changed on both sides" → the ship PR conflicts on those two files on EVERY ship until local re-syncs. `sync-local-to-main.sh` fetches `main`, merges it into local (native git, no API ref-writes), auto-resolves ONLY the two version files in favour of local (the higher, freshly-bumped version), and HARD-STOPS on any other conflict or a dirty tree. Run it after a successful ship, or any time the PR shows a conflict on `config.go`/`sonar-project.properties`. Safe to run anytime; it is a no-op when already in sync.
  - **Ephemeral ship branches**: each run pushes the workspace HEAD to a brand-new `ship/<timestamp>-<sha>` branch (never the persistent `replit-agent` branch). A fresh branch cannot be non-fast-forward, so the chronic `! [rejected] (fetch first)` divergence is structurally impossible. The branch is squash-merged into `main` and auto-deleted. `--no-main` pushes a `snapshot/<ts>-<sha>` backup branch instead and stops (no PR).
  - **Squash-only, GitHub-signed main**: merge-commit & rebase are disabled at repo + ruleset level; GitHub signs server-side squash commits, so `main` stays 100% `verified:true` WITHOUT contributors signing. This replaced the old `required_signatures` rule, which was incompatible with Replit's unsigned checkpoint commits and silently blocked every PR. NEVER switch the ship flow back to `--rebase`/`--merge` (they carry unsigned commits onto `main` → rejected "base branch policy prohibits the merge").
  - **Tokens**: the git push uses `GH_SYNC_TOKEN` (push scope); the PR create + auto-merge use `ALL_GH` (PR scope — `GH_SYNC_TOKEN` cannot create PRs). Both are repo/PR API ops, not git-ref writes (Repo Sync Law compliant).
  - Required status checks on `main`: `CodeQL` + `Build & Test`, **non-strict** (strict deadlocks this flow — local never contains main's squash tip, so a strict "up to date" check can never pass). Force-push blocked, linear history required, mainrule bypass empty. Verify the whole policy any time: `export GH_TOKEN="$ALL_GH"; bash scripts/check-merge-policy.sh` (drift guard). Use `bash scripts/git-push.sh --no-main` for branch-only push (no PR).
