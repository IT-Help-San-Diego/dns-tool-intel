...
- **THE THREE-COMMAND PLANS** (canonical sequences — copy to notes):
  ...
  **A. Ship a dev bump to `origin/main`** (post-branch-protection 2026-05-16):
  ...
  3. bash scripts/git-push.sh         # push to a FRESH ship/<ts> branch → open PR → auto-merge --squash on green → branch auto-deleted
  4. bash scripts/sync-local-to-main.sh   # AFTER merge: re-base local onto main so the NEXT ship has no version conflict
  ```
