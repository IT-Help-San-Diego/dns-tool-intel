...
- **Owl Semaphore**: Four-state iconographic system (NORMATIVE / NON-NORMATIVE / CRITICAL / METACOGNITIVE) using the Klein four-group V₄ {I, σᵥ, C₂, σₕ} (subgroup of O(2)) on the Owl of Athena. **Authoritative source = public GitHub repo `IT-Help-San-Diego/owl-semaphore` (v3.0.0; version DOI 10.5281/zenodo.20468727, concept DOI 19473697)** — that repo, NOT local build-time generation, is the canonical art + spec. Local `/owl-semaphore` page mirrors it; the 540px composites in `static/exports/owl-semaphore/` + `go-server/static/exports/owl-semaphore/` are copied from the repo's `assets/releases/540/`, and the responsive `derived/` variants are produced by the MANUAL-ONLY `scripts/generate-owl-derived.sh` (the build.sh auto-gen hook was removed). NOTE: the delivered v3 art was recolored vs. the repo's prose color tables — trust the actual composite pixels — sample the 540px composites for exact values (verified dominants: META field tan #786042, lavender owl #b89cec, amethyst #8c4191 ring; CRIT oxblood field #371218, red owl #e25c60; NONNORM gold field #d5af64, teal owl #4fb5b5 — NOT the docs' violet-black #1A1020). Page template: `go-server/templates/owl_semaphore.html`. Legacy export-kit spec: `static/exports/dead-owl-icon/EXPORT-KIT.md`.
