...
- **THE THREE-COMMAND PLANS** (canonical sequences — copy to notes):
  ...
  **A. Ship a dev bump to `origin/main`** (post-branch-protection 2026-05-16):
  ...
  3. bash scripts/git-push.sh         # push branch → open PR → wait for required checks → auto-merge --squash
  ```
  `git-push.sh` handles the PR lifecycle automatically. No direct push to main. No API ref-writes. **Merges are SQUASH-ONLY** (merge-commit & rebase are disabled at repo + ruleset level): GitHub signs server-side squash commits, so `main` stays 100% `verified:true` WITHOUT contributors signing — this replaces the old `required_signatures` rule, which was structurally incompatible with Replit's unsigned checkpoint commits and silently blocked every PR (removed 2026-06-08). NEVER switch the ship flow back to `--rebase`/`--merge`: they carry unsigned commits onto `main` and GitHub rejects them ("base branch policy prohibits the merge"). Required status checks on `main`: `CodeQL` + `Build & Test` (strict — branch must be up to date). Force-push blocked, linear history required, mainrule bypass empty. Verify the whole policy any time with `export GH_TOKEN="$ALL_GH"; bash scripts/check-merge-policy.sh` (drift guard). Use `bash scripts/git-push.sh --no-main` for branch-only push (no PR).
