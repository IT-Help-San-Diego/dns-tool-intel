...
- **CITATION.cff / codemeta.json — HANDS OFF**: ORCID-linked Zenodo artifacts. Never modify automatically. License must remain `BUSL-1.1`. Concept DOI `10.5281/zenodo.19468134` NEVER changes (active chain since 2026-04-08 migration commit a8c9742cb; the earlier concept `10.5281/zenodo.18854899` is a dormant duplicate lineage on Zenodo, last version v26.38.35 — do NOT revert repo files to it). CI guard `.github/workflows/guard_citation.yml` blocks license/file regressions (it does NOT check the DOI — the hands-off rule is the only DOI protection).
